...
from (values
  (1,'Свет разума','+2 к Интеллекту.','{"intellect":2}'),
  (2,'Стойка хранителя','Блок снижает входящий урон ещё на 6 п.п.','{"guard_boost":6}'),
  (3,'Ясность долга','+2 к Удаче.','{"luck":2}'),
  (4,'Священная стойкость','+3% сопротивления режущему, колющему и дробящему урону.','{"resistances":{"slashing":3,"piercing":3,"blunt":3}}'),
  (5,'Благодать шага','Уклонение +5 п.п.','{"evasion_chance":5}'),
  (6,'Милость Света','Эффективность лечебных заклинаний +6%.','{"healing_spell_bonus":6}'),
  (7,'Закалённая вера','+3 к Живучести.','{"vitality":3}'),
  (8,'Стена Света','Эффективность защитных заклинаний-щитов +8%.','{"shield_spell_bonus":8}'),
  (9,'Последний рубеж','При HP ниже 50% входящий урон снижается ещё на 5 п.п.','{"low_hp_50_damage_reduction":5}')
) as v(level,title,description,modifiers)
where religion_level_perks.religion_slug='path_of_light'
  and religion_level_perks.level=v.level;

create or replace function private.character_religion_modifier_number(
  p_character_id uuid,
  p_key text
)
returns integer
...
as $$
  select case
    when jsonb_typeof(private.character_religion_modifiers(p_character_id)->p_key)='number'
      then coalesce((private.character_religion_modifiers(p_character_id)->>p_key)::integer,0)
    else 0
  end;
$$;

revoke all on function private.character_religion_modifier_number(uuid,text) from public, anon, authenticated;
...
begin
  ...
  if player_stunned then
    ...
    if p_mode='support_spell' then
      ...
      if stats.mana_current<spell.mana_cost then raise exception 'NOT_ENOUGH_MANA'; end if;
  ...
  if not guard_active and not player_stunned and not player_support_action then
    ...
    if p_mode='physical' then
      ...
      type_damage_bonus:=private.character_damage_bonus(encounter.character_id,player_damage_type);
      raw_damage:=greatest(1,round(raw_damage*(100+stats.all_damage_bonus_percent+stats.magic_damage_bonus_percent+type_damage_bonus)/100.0)::integer);
    end if;
  ...
  then
    ...
    if special_attack_active and encounter.enemy_special_kind='heal' then
      ...
      player_defense_value:=greatest(0,round(player_defense_value*(100+private.character_defense_percent(encounter.character_id))/100.0)::integer);
...
begin
  ...
  if stats.level<spell.required_level then raise exception 'LEVEL_TOO_LOW'; end if;
  if actor_state.mana_current<spell.mana_cost then raise exception 'NOT_ENOUGH_MANA'; end if;

  select c.name into actor_name
  from public.characters c
  where c.id=p_character_id;

  actor_mana_after:=actor_state.mana_current-spell.mana_cost;
  ...
  if spell.spell_kind='damage' then
    ...
    if spell.spell_kind='heal' then
      ...
      action_message:=actor_name||' накладывает «'||spell.name||'» на '
        ...
        ||guard_value||'%. Мана: -'||spell.mana_cost||'.';
      ...
      action_message:=actor_name||' применяет «'||spell.name||'» на '
        ...
        ||'противник выбирает его целью с шансом '||spell.support_value||'%. Мана: -'||spell.mana_cost||'.';
      ...
      action_message:=actor_name||' применяет «'||spell.name||'» на '
        ...
        ||spell.support_turns||' атаки. Мана: -'||spell.mana_cost||'.';
    end if;
...
begin
  ...
  if enemy_stunned then
    ...
    target_defense:=greatest(0,round(target_defense*(100+private.character_defense_percent(target_id))/100.0)::integer);
...
begin
  ...
  if stunned then
    ...
    if p_action='physical' then
      ...
      if actor.mana_current<spell.mana_cost then raise exception 'NOT_ENOUGH_MANA'; end if;
      if spell.spell_kind not in ('damage','heal','guard','cleanse','buff') then raise exception 'SPELL_NOT_COMBAT_USABLE'; end if;

      actor.mana_current:=actor.mana_current-spell.mana_cost;
      action_type_value:='spell_'||spell.slug;
      action_label:=spell.name;
      ...
      if spell.spell_kind='heal' then
        ...
        message_text:=coalesce(actor_name,'Персонаж')||' использует «'||spell.name||'» и восстанавливает '
          ||heal_value||' HP. Мана: -'||spell.mana_cost||'.';
      elsif spell.spell_kind='guard' then
        guard_value:=least(
          85,
          greatest(
        ...
        actor_reduction:=0;
        message_text:=coalesce(actor_name,'Персонаж')||' использует «'||spell.name
          ||'» и снимает негативные эффекты: '||cleanse_count||'. Мана: -'||spell.mana_cost||'.';
        ...
        message_text:=coalesce(actor_name,'Персонаж')||' использует «'||spell.name
          ...
          ||actor.spell_damage_bonus_hits||' атак. Мана: -'||spell.mana_cost||'.';
      else
    ...
    if raw_damage>0 then
      ...
      message_text:=case
        ...
        ||case when empower_used>0 then ' Магическое усиление: +'||empower_used||'%.' else '' end
        ||case when p_action='spell' then ' Мана: -'||spell.mana_cost||'.' else '' end
        ||case when first_physical_strike_active then ' Первый удар катаны: база ×'||trim(to_char(first_strike_multiplier,'FM9990.0'))||', бонусная часть ×'||trim(to_char(first_bonus_multiplier,'FM9990.0'))||'.' else '' end
        ||case when katana_rhythm_bonus>0 then ' Нарастающий ритм: +'||katana_rhythm_bonus||'% урона.' else '' end
        ||case when critical_hits>0 then ' Критических попаданий: '||critical_hits||'.' else '' end
