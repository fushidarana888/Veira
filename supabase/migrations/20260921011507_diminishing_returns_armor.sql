create or replace function private.armor_damage_reduction_percent(p_armor numeric)
returns numeric
language sql
immutable
...
as $function$
  select least(
    ...
    greatest(
      0.0,
      80.0 * greatest(0.0,coalesce(p_armor,0.0))
      / (greatest(0.0,coalesce(p_armor,0.0)) + 60.0)
    )
  );
$function$;

create or replace function private.damage_after_armor(p_damage numeric,p_armor numeric)
returns integer
language sql
immutable
...
as $function$
  select greatest(
    0,
    round(
      greatest(0.0,coalesce(p_damage,0.0))
      * (100.0-private.armor_damage_reduction_percent(p_armor))
      / 100.0
    )::integer
  );
$function$;

CREATE OR REPLACE FUNCTION private.weapon_family_physical_raw_damage_for_family(p_family text, p_physical_power integer, p_target_defense integer, p_target_hp_max integer, p_variance integer DEFAULT 0)
 RETURNS integer
 LANGUAGE plpgsql
 IMMUTABLE
 SET search_path TO 'pg_catalog'
AS $function$
declare
  effective_defense integer:=greatest(0,coalesce(p_target_defense,0));
  result integer;
  hp_bonus integer:=0;
begin
  if p_family='rapier' then
    effective_defense:=floor(effective_defense*0.90)::integer;
  elsif p_family='spear' then
    effective_defense:=ceil(effective_defense*1.20)::integer;
  end if;

  result:=greatest(
    1,
    private.damage_after_armor(
      coalesce(p_physical_power,0)+coalesce(p_variance,0),
      effective_defense
    )
  );

  if p_family='dagger' then
    result:=greatest(1,round(result*0.80)::integer);
  elsif p_family in ('axe','battleaxe') then
    hp_bonus:=least(
      20,
      greatest(0,floor(greatest(0,coalesce(p_target_hp_max,0)-100)/100.0)::integer)
    );
    result:=greatest(1,round(result*(100+hp_bonus)/100.0)::integer);
  end if;
...
CREATE OR REPLACE FUNCTION private.choose_bow_autobattle(p_encounter_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
declare
  ...
  enemy_hp_percent integer:=100;
  selected_distance text:='medium';
  distance_multiplier numeric:=1.0;
  resistance integer:=0;
  total_bonus integer:=0;
  fast_raw integer:=0;
  full_raw integer:=0;
  fast_score integer:=0;
  full_score integer:=0;
  close_fast_score integer:=0;
  decision_score integer:=0;
  chosen_action text:='physical';
  reason text:='balanced';
begin
  ...
  if bow_family is null or bow_family not in ('short_bow','long_bow') then
    return jsonb_build_object(
      'is_bow',false,
      'distance',encounter.player_bow_distance,
      'action','physical',
      'score',0,
      'fast_score',0,
      'full_score',0,
  ...
  hp_percent:=case when stats.hp_max<=0 then 0 else floor(stats.hp_current*100.0/stats.hp_max)::integer end;
  enemy_hp_percent:=case when encounter.enemy_hp_max<=0 then 0 else floor(encounter.enemy_hp_current*100.0/encounter.enemy_hp_max)::integer end;
  resistance:=private.damage_resistance_percent(encounter.enemy_resistances,stats.weapon_damage_type);
  total_bonus:=coalesce(stats.all_damage_bonus_percent,0)
    +coalesce(stats.physical_damage_bonus_percent,0)
    +coalesce(private.character_damage_bonus(encounter.character_id,stats.weapon_damage_type),0)
    +case when encounter.is_boss then coalesce(stats.boss_damage_bonus_percent,0) else 0 end;

  close_fast_score:=private.autobattle_estimated_damage(
    greatest(1,private.damage_after_armor(
      round(stats.physical_power*private.bow_distance_multiplier('close'))::integer,
  ...
  if encounter.player_bow_draw_pending then
    selected_distance:=encounter.player_bow_distance;
    chosen_action:='physical';
    reason:='full_draw_release';
  elsif encounter.enemy_special_charging and encounter.enemy_special_kind='attack' then
    selected_distance:='far';
    reason:='enemy_special_attack';
  elsif hp_percent<=35 then
    selected_distance:='far';
    reason:='low_hp';
  elsif encounter.is_boss and hp_percent<=55 then
    selected_distance:='far';
    reason:='boss_pressure';
  elsif encounter.enemy_attack>=greatest(1,round(stats.hp_current*0.30)::integer) then
    selected_distance:='far';
    reason:='high_incoming_threat';
  elsif close_fast_score>=encounter.enemy_hp_current then
    selected_distance:='close';
    reason:='close_finisher';
  elsif not encounter.is_boss and hp_percent>=80 and enemy_hp_percent<=50 then
    selected_distance:='close';
    reason:='safe_pressure';
  else
    selected_distance:='medium';
    reason:='balanced';
  end if;

  distance_multiplier:=private.bow_distance_multiplier(selected_distance);

  fast_raw:=greatest(
    1,
    private.damage_after_armor(
      round(stats.physical_power*distance_multiplier)::integer,
      encounter.enemy_defense
    )
  );
  full_raw:=greatest(
    1,
    private.damage_after_armor(
      round(stats.physical_power*distance_multiplier*1.60)::integer,
      floor(encounter.enemy_defense*(100-bow_penetration)/100.0)::integer
    )
  );

  fast_score:=private.autobattle_estimated_damage(fast_raw,resistance,total_bonus);
  full_score:=private.autobattle_estimated_damage(full_raw,resistance,total_bonus);

  if encounter.player_bow_draw_pending then
    chosen_action:='physical';
  elsif bow_family='long_bow' then
    chosen_action:='bow_draw';
    reason:=reason||'_long_bow';
  elsif encounter.enemy_special_charging and encounter.enemy_special_kind='attack' then
    chosen_action:='physical';
    reason:=reason||'_fast';
  elsif hp_percent<=45 then
    chosen_action:='physical';
    reason:=reason||'_fast';
  elsif fast_score>=encounter.enemy_hp_current then
    chosen_action:='physical';
    reason:=reason||'_finisher';
  elsif enemy_hp_percent<=25 then
    chosen_action:='physical';
    reason:=reason||'_finish_pressure';
  elsif encounter.is_boss
        and hp_percent>=65
        and enemy_hp_percent>=35
        and full_score>=ceil(fast_score*1.35)::integer
  then
    chosen_action:='bow_draw';
    reason:=reason||'_boss_full';
  elsif encounter.enemy_defense>=ceil(stats.physical_power*0.35)::integer
        and hp_percent>=55
        and full_score>=ceil(fast_score*1.30)::integer
  then
    chosen_action:='bow_draw';
    reason:=reason||'_armor_full';
  elsif hp_percent>=80
        and enemy_hp_percent>=70
        and full_score>=ceil(fast_score*1.55)::integer
  then
    chosen_action:='bow_draw';
    reason:=reason||'_safe_full';
  else
    chosen_action:='physical';
    reason:=reason||'_fast';
  end if;
  ...
  decision_score:=case
    when chosen_action='bow_draw' then greatest(1,round(full_score*0.65)::integer)
    else fast_score
  end;
