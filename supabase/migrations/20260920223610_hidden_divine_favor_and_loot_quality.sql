...
set favor=greatest(0,least(100,favor))
where favor<0 or favor>100;

alter table public.character_religion_progress
  drop constraint if exists character_religion_progress_favor_check;

alter table public.character_religion_progress
  add constraint character_religion_progress_favor_check
  check (favor between 0 and 100);
...
CREATE OR REPLACE FUNCTION private.character_current_favor(p_character_id uuid)
 RETURNS integer
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public', 'private'
...
CREATE OR REPLACE FUNCTION private.character_hidden_favor_crit_bonus(p_character_id uuid)
 RETURNS numeric
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public', 'private'
AS $function$
  select case when coalesce(private.character_current_favor(p_character_id),0)>=75 then 2.0 else 0.0 end;
$function$;

CREATE OR REPLACE FUNCTION private.character_hidden_favor_evasion_bonus(p_character_id uuid)
 RETURNS integer
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public', 'private'
AS $function$
  select case when coalesce(private.character_current_favor(p_character_id),0)>=25 then 2 else 0 end;
$function$;
...
AS $function$
  select least(
    ...
    greatest(0,coalesce(s.luck,0))*2.0
      +case when coalesce(private.character_current_favor(p_character_id),0)>=50 then 25.0 else 0.0 end
  )
  from private.get_character_combat_stats(p_character_id) s;
$function$;

CREATE OR REPLACE FUNCTION private.dungeon_quality_loot_chance(p_character_id uuid, p_item_definition_id uuid, p_base_chance numeric)
 RETURNS numeric
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public', 'private'
AS $function$
  select least(
    ...
    greatest(
      0.0,
      coalesce(p_base_chance,0)
      *case
        when private.is_quality_dungeon_gear(p_item_definition_id)
          then (100.0+coalesce(private.character_loot_quality_bonus_percent(p_character_id),0))/100.0
        else 1.0
      end
    )
  );
...
begin
  ...
  set faith_points=greatest(0,faith_points-v_faith),
      favor=greatest(0,favor-v_favor),
      updated_at=now()
  where character_id=p_character_id and religion_slug=v_religion;
end;
$function$;

CREATE OR REPLACE FUNCTION private.is_quality_dungeon_gear(p_item_definition_id uuid)
 RETURNS boolean
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public', 'private'
AS $function$
  select coalesce(
    (
      select i.category::text in ('weapon','armor','accessory')
         and private.item_rarity_rank(i.rarity)>=private.item_rarity_rank('rare'::public.item_rarity)
...
CREATE OR REPLACE FUNCTION private.record_religion_event(p_character_id uuid, p_event_type text, p_source_key text, p_metadata jsonb DEFAULT '{}'::jsonb)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public', 'private'
...
begin
  ...
  set faith_points=greatest(0,least(2200,faith_points+v_actual_faith)),
      favor=greatest(0,least(100,favor+v_base_favor)),
      updated_at=now()
  where character_id=p_character_id and religion_slug=v_religion;
  ...
  if v_oath_id is not null then
    ...
    then
      ...
      set faith_points=greatest(0,faith_points-v_failure_faith),
          favor=greatest(0,favor-v_failure_favor),
          updated_at=now()
      where character_id=p_character_id and religion_slug=v_religion;
...
CREATE OR REPLACE FUNCTION private.roll_dungeon_quality_loot(p_character_id uuid, p_item_definition_id uuid, p_base_chance numeric)
 RETURNS boolean
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public', 'private'
AS $function$
begin
  return random()*100 < private.dungeon_quality_loot_chance(
    p_character_id,
...
CREATE OR REPLACE FUNCTION public.get_character_religion_oaths_v2(p_character_id uuid)
 RETURNS TABLE(oath_id uuid, name text, description text, target_count integer, progress_count integer, faith_reward integer, status text, active_assignment_id uuid, cooldown_remaining_seconds bigint)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public', 'private'
AS $function$
...
begin
  ...
  select
    ...
    d.faith_reward,coalesce(a.status,'available'),
...
CREATE OR REPLACE FUNCTION public.gm_get_character_hidden_favor(p_character_id uuid)
 RETURNS TABLE(religion_slug text, religion_name text, favor integer)
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public', 'private'
AS $function$
begin
  ...
  return query
  select
    cr.current_religion_slug,
    rd.name,
    greatest(0,least(100,coalesce(cp.favor,0)))::integer
  from public.character_religions cr
  join public.religion_definitions rd on rd.slug=cr.current_religion_slug
  left join public.character_religion_progress cp
    on cp.character_id=cr.character_id
   and cp.religion_slug=cr.current_religion_slug
  where cr.character_id=p_character_id
...
CREATE OR REPLACE FUNCTION public.gm_set_character_hidden_favor(p_character_id uuid, p_favor integer)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public', 'private'
...
begin
  ...
  select current_religion_slug into v_slug
  from public.character_religions
  where character_id=p_character_id;

  if v_slug is null then raise exception 'NO_ACTIVE_RELIGION'; end if;

  insert into public.character_religion_progress(character_id,religion_slug,favor)
  values(p_character_id,v_slug,greatest(0,least(100,coalesce(p_favor,0))))
  on conflict(character_id,religion_slug) do update
  set favor=greatest(0,least(100,coalesce(p_favor,0))),
      updated_at=now();

  insert into public.gm_audit_log(actor_user_id,action,target_type,target_id,details)
  values(
    auth.uid(),'religion.hidden_favor_set','character',p_character_id::text,
    jsonb_build_object('religion_slug',v_slug,'favor',greatest(0,least(100,coalesce(p_favor,0))))
  );
end;
$function$;
...
revoke all on function private.character_current_favor(uuid) from public,anon,authenticated;
revoke all on function private.character_hidden_favor_evasion_bonus(uuid) from public,anon,authenticated;
revoke all on function private.character_hidden_favor_crit_bonus(uuid) from public,anon,authenticated;
revoke all on function private.is_quality_dungeon_gear(uuid) from public,anon,authenticated;
revoke all on function private.character_loot_quality_bonus_percent(uuid) from public,anon,authenticated;
revoke all on function private.dungeon_quality_loot_chance(uuid,uuid,numeric) from public,anon,authenticated;
revoke all on function private.roll_dungeon_quality_loot(uuid,uuid,numeric) from public,anon,authenticated;

revoke all on function public.get_religion_catalog_v2(uuid) from public,anon;
grant execute on function public.get_religion_catalog_v2(uuid) to authenticated;
revoke all on function public.get_character_religion_oaths_v2(uuid) from public,anon;
grant execute on function public.get_character_religion_oaths_v2(uuid) to authenticated;

revoke all on function public.gm_get_character_hidden_favor(uuid) from public,anon;
grant execute on function public.gm_get_character_hidden_favor(uuid) to authenticated;
revoke all on function public.gm_set_character_hidden_favor(uuid,integer) from public,anon;
grant execute on function public.gm_set_character_hidden_favor(uuid,integer) to authenticated;

revoke all on function public.get_religion_catalog(uuid) from public,anon,authenticated;
revoke all on function public.get_character_religion_oaths(uuid) from public,anon,authenticated;
