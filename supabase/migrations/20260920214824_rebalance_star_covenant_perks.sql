...
from (values
  (1,'Первая звезда','+2 к Интеллекту.','{"intellect":2}'),
  (2,'Отголосок эфира','Успешный удар возвращает 3 маны.','{"mana_on_hit":3}'),
  (3,'Знак пути','+2 к Удаче.','{"luck":2}'),
  (4,'Звёздный покров','Магическая защита +3%.','{"magic_defense_percent":3}'),
  (5,'Милость созвездий','Эффективность лечебных заклинаний +5%.','{"healing_spell_bonus":5}'),
  (6,'Эфирная экономия','Расход маны на все изученные заклинания снижен на 7%.','{"spell_mana_cost_reduction_percent":7}'),
  (7,'Созвездие силы','Весь магический урон +3%.','{"magic_damage_bonus":3}'),
  (8,'Арканный резонанс','Урон заклинаний семейства «Арканная» +5%.','{"magic_family_damage_bonuses":{"arcane":5}}'),
  (9,'Небесная защита','Магическая защита +5%.','{"magic_defense_percent":5}')
) as v(level,title,description,modifiers)
where religion_level_perks.religion_slug='star_covenant'
...
CREATE OR REPLACE FUNCTION private.character_effective_spell_mana_cost(p_character_id uuid, p_spell_id uuid)
 RETURNS integer
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public', 'private'
AS $function$
  select case
    when coalesce(s.mana_cost,0)<=0 then 0
    else greatest(
      1,
      round(
        s.mana_cost
        *(
          100-greatest(
            ...
            least(
              80,
              private.character_religion_modifier_number(
                p_character_id,
                'spell_mana_cost_reduction_percent'
              )
            )
          )
        )
...
CREATE OR REPLACE FUNCTION private.character_spell_family_damage_bonus_percent(p_character_id uuid, p_spell_id uuid)
 RETURNS integer
 LANGUAGE sql
 STABLE SECURITY DEFINER
 SET search_path TO 'pg_catalog', 'public', 'private'
AS $function$
  with state as (
    select
      cr.current_religion_slug as religion_slug,
      private.religion_level(coalesce(cp.faith_points,0)) as religion_level
    from public.character_religions cr
    left join public.character_religion_progress cp
      on cp.character_id=cr.character_id
     and cp.religion_slug=cr.current_religion_slug
    where cr.character_id=p_character_id
      and cr.current_religion_slug is not null
  ),
  active_perks as (
    select p.modifiers
    from state s
    join public.religion_level_perks p
      on p.religion_slug=s.religion_slug
     and p.level<=s.religion_level
  ),
  spell_families as (
    select sf.family_slug
  ...
  select least(
    ...
    coalesce(sum(
      case
        when jsonb_typeof(ap.modifiers->'magic_family_damage_bonuses'->sf.family_slug)='number'
          then (ap.modifiers->'magic_family_damage_bonuses'->>sf.family_slug)::integer
        else 0
      end
    ),0)::integer
  )
  from active_perks ap
  cross join spell_families sf;
$function$;
...
begin
  ...
  if not guard_active and not player_stunned and not player_support_action then
    ...
    if encounter.is_boss and stats.boss_damage_bonus_percent>0 then
      raw_damage:=greatest(1,round(raw_damage*(100+stats.boss_damage_bonus_percent)/100.0)::integer);
    end if;
...
declare
  ...
  opponent_stats record;
  first_character uuid;
  first_name text;
begin
  if caller_id is null then raise exception 'AUTH_REQUIRED'; end if;

  select d0.* into d
  from public.pvp_duels d0
  join public.characters c on c.id=d0.opponent_character_id
  where d0.id=p_duel_id and c.owner_user_id=caller_id
  for update of d0;

  if d.id is null then raise exception 'DUEL_NOT_FOUND'; end if;
  if d.status<>'pending' then raise exception 'DUEL_NOT_PENDING'; end if;

  if not coalesce(p_accept,false) then
    update public.pvp_duels
    set status='declined',ended_at=now(),updated_at=now(),finish_reason='declined'
    where id=d.id;
    return d.id;
  end if;

  if private.character_busy_for_duel(d.challenger_character_id,d.id)
     or private.character_busy_for_duel(d.opponent_character_id,d.id)
  then raise exception 'PLAYER_BUSY'; end if;

  select * into challenger_stats from private.get_character_combat_stats(d.challenger_character_id);
  select * into opponent_stats from private.get_character_combat_stats(d.opponent_character_id);
  if challenger_stats.level is null or opponent_stats.level is null then
    raise exception 'COMBAT_STATS_NOT_FOUND';
  end if;

  begin
    insert into public.pvp_duel_locks(character_id,duel_id)
    values(d.challenger_character_id,d.id),(d.opponent_character_id,d.id);
  exception when unique_violation then
    raise exception 'PLAYER_BUSY';
  end;

  insert into public.pvp_duel_states(
    duel_id,character_id,side,level,hp_current,hp_max,mana_current,mana_max,
    physical_power,magic_power,defense,physical_defense,magic_defense,initiative,luck,weapon_damage_type,magic_damage_type,
    damage_resistances,damage_bonuses,lifesteal_percent,mana_on_hit,damage_vs_wounded_percent,
    guard_boost_percent,all_damage_bonus_percent,physical_damage_bonus_percent,
    magic_damage_bonus_percent,low_hp_damage_reduction_percent
  )
  values
  (
    d.id,d.challenger_character_id,'challenger',challenger_stats.level,
    challenger_stats.hp_max,challenger_stats.hp_max,
    challenger_stats.mana_max,challenger_stats.mana_max,
    challenger_stats.physical_power,challenger_stats.magic_power,
    challenger_stats.defense,
    greatest(0,round(private.character_physical_defense(challenger_stats.level,challenger_stats.vitality,challenger_stats.agility)*(100+private.character_defense_percent(d.challenger_character_id))/100.0)::integer),
    greatest(0,round(private.character_magic_defense(challenger_stats.level,challenger_stats.vitality,challenger_stats.intellect)*(100+private.character_defense_percent(d.challenger_character_id)+private.character_religion_modifier_number(d.challenger_character_id,'magic_defense_percent'))/100.0)::integer),
    challenger_stats.initiative,challenger_stats.luck,
    challenger_stats.weapon_damage_type,challenger_stats.magic_damage_type,
    challenger_stats.damage_resistances,private.character_damage_bonuses(d.challenger_character_id),challenger_stats.lifesteal_percent,
    challenger_stats.mana_on_hit,challenger_stats.damage_vs_wounded_percent,
    challenger_stats.guard_boost_percent,challenger_stats.all_damage_bonus_percent,
    challenger_stats.physical_damage_bonus_percent,challenger_stats.magic_damage_bonus_percent,
    challenger_stats.low_hp_damage_reduction_percent
  ),
  (
    d.id,d.opponent_character_id,'opponent',opponent_stats.level,
    opponent_stats.hp_max,opponent_stats.hp_max,
    opponent_stats.mana_max,opponent_stats.mana_max,
    opponent_stats.physical_power,opponent_stats.magic_power,
    opponent_stats.defense,
    greatest(0,round(private.character_physical_defense(opponent_stats.level,opponent_stats.vitality,opponent_stats.agility)*(100+private.character_defense_percent(d.opponent_character_id))/100.0)::integer),
    greatest(0,round(private.character_magic_defense(opponent_stats.level,opponent_stats.vitality,opponent_stats.intellect)*(100+private.character_defense_percent(d.opponent_character_id)+private.character_religion_modifier_number(d.opponent_character_id,'magic_defense_percent'))/100.0)::integer),
    opponent_stats.initiative,opponent_stats.luck,
    opponent_stats.weapon_damage_type,opponent_stats.magic_damage_type,
    opponent_stats.damage_resistances,private.character_damage_bonuses(d.opponent_character_id),opponent_stats.lifesteal_percent,
    opponent_stats.mana_on_hit,opponent_stats.damage_vs_wounded_percent,
    opponent_stats.guard_boost_percent,opponent_stats.all_damage_bonus_percent,
    opponent_stats.physical_damage_bonus_percent,opponent_stats.magic_damage_bonus_percent,
    opponent_stats.low_hp_damage_reduction_percent
  );

  if challenger_stats.initiative>opponent_stats.initiative then
    first_character:=d.challenger_character_id;
  elsif opponent_stats.initiative>challenger_stats.initiative then
    first_character:=d.opponent_character_id;
  else
    first_character:=case when random()<0.5 then d.challenger_character_id else d.opponent_character_id end;
  end if;

  update public.pvp_duels
  set status='active',current_turn_character_id=first_character,round=1,
      started_at=now(),turn_started_at=now(),updated_at=now()
  where id=d.id;

  update public.pvp_duels
  set status='cancelled',ended_at=now(),updated_at=now(),finish_reason='another_duel_started'
  where id<>d.id and status='pending'
    and (
      challenger_character_id in (d.challenger_character_id,d.opponent_character_id)
      or opponent_character_id in (d.challenger_character_id,d.opponent_character_id)
    );

  select name into first_name from public.characters where id=first_character;

  insert into public.pvp_duel_turns(duel_id,round,actor_character_id,action_type,message)
  values(d.id,0,null,'start','Дуэль началась. Первый ход: '||coalesce(first_name,'неизвестный персонаж')||'.');
...
begin
  ...
  select
    ...
    private.character_effective_spell_mana_cost(p_character_id,s.id),s.required_level,s.power_multiplier,s.flat_power,
...
revoke all on function private.character_effective_spell_mana_cost(uuid,uuid)
from public, anon, authenticated;
revoke all on function private.character_spell_family_damage_bonus_percent(uuid,uuid)
from public, anon, authenticated;
