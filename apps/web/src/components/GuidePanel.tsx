...
const sections: Array<{ id: GuideSection; label: string; description: string }> = [
  ...
  { id: 'ruins', label: 'Руины', description: 'Исследование, награды и находки' },
...
const familyMechanics: Array<{ family: string; title: string; text: string }> = [
  ...
  },
  {
    family: 'greatsword',
    title: 'Двуручный меч',
    text: 'Каждый некритический физический удар добавляет +15 п.п. к следующему шансу крита. Крит сбрасывает накопление; общий шанс крита ограничен 75%.',
  },
  {
    family: 'staff',
    title: 'Боевой посох',
    ...
    <section className="guide-shell">
      ...
      <div className="guide-layout">
        ...
        <div className="guide-content">
          ...
          {!loading && catalog && section === 'mechanics' && (
            <>
              ...
              <div className="guide-rule-grid">
                ...
                <GuideRule title="Физическая броня">
                  Числовая броня: <b>ЖИВ ×2 + ЛОВ ×0,5 + уровень</b>, затем применяются бонусы экипировки,
                  расы и эффектов. Она больше не вычитается из урона напрямую.
                </GuideRule>
                <GuideRule title="Магическая броня">
                  Числовая броня: <b>ЖИВ + ИНТ + уровень</b>. Работает по той же кривой против
                  магического и стихийного входящего урона.
                </GuideRule>
                <GuideRule title="Снижение урона бронёй">
                  Формула: <b>80% × Броня / (Броня + 60)</b>. Примеры:
                  10 брони ≈ <b>11,4%</b>, 20 = <b>20%</b>, 30 ≈ <b>26,7%</b>,
                  50 ≈ <b>36,4%</b>, 100 = <b>50%</b>. Предел — 80%, но он только асимптотический.
                </GuideRule>
                <GuideRule title="Убывающая отдача">
                  Каждые следующие очки брони дают меньше процентов, чем предыдущие.
                  Поэтому высокая броня остаётся ценной, но больше не превращает слабые атаки в постоянный 1 урон.
                </GuideRule>
                <GuideRule title="Инициатива и частота ходов">
                  Инициатива считается из <b>ЛОВ ×2 + УДА</b> и дополнительных расовых эффектов.
                  После каждого полноценного действия живой персонаж получает <b>темп действия</b>.
                  Темп получают все персонажи, но высокая инициатива накапливает его быстрее относительно инициативы противника.
                  При <b>100 темпа</b> персонаж получает ещё одно полноценное действие до ответа противника.
                  За одно действие начисляется от <b>6 до 25 темпа</b>, поэтому медленные персонажи тоже иногда получают
                  дополнительный ход, а быстрые делают это заметно чаще, но не переходят к постоянным двойным ходам.
                  Оглушённый или выведенный из боя персонаж темп за пропущенное действие не получает.
                </GuideRule>
          ...
          {!loading && catalog && section === 'armor' && (
            <>
              <GuideHeading
                ...
                text="Броня даёт числовую физическую или магическую защиту, которая превращается в процент снижения урона с убывающей отдачей. Отдельно работают сопротивления конкретным типам урона и уникальные свойства."
              />
          ...
          {!loading && catalog && section === 'ruins' && (
            <>
              <GuideHeading
                eyebrow="РУИНЫ"
                title="Одноразовое глубокое исследование"
                text="Руины — не второй данж, а исследовательская активность. После открытия сектора их можно один раз подробно изучить каждым персонажем и получить гарантированную награду плюс случайную находку."
              />

              <div className="guide-rule-grid">
                <GuideRule title="Время исследования">
                  Базовое время — <b>2 часа</b>. Бонусы скорости исследования сокращают его так же, как время обычных экспедиций.
                  Пока идёт таймер, можно заниматься лёгкими активностями; второе исследование или тяжёлый PvE начать нельзя.
                </GuideRule>
                <GuideRule title="Гарантированная награда">
                  Всегда выдаются золото и опыт. Опасность увеличивает обе награды:
                  опасность 0 — <b>20–30 золота и 25–40 опыта</b>,
                  опасность 5 — <b>70–90 золота и 115–145 опыта</b>,
                  опасность 10 — <b>120–150 золота и 205–250 опыта</b>.
                </GuideRule>
                <GuideRule title="40% · тайник с материалами">
                  Находится материал, связанный с местностью: например Солнечное стекло в пустыне,
                  Морозный кристалл в тундре или Осколок каменного ядра в горах.
                </GuideRule>
                <GuideRule title="25% · забытые припасы">
                  Выдаётся зелье ОЗ или маны. Чем опаснее руины, тем выше его качество:
                  обычные на низкой опасности, необычные на средней и редкие на высокой.
                </GuideRule>
                <GuideRule title="17% · утраченное знание">
                  В руинах опасности 0–4 находится боевой свиток.
                  Начиная с <b>опасности 5</b> вместо него можно найти свиток постоянного изучения базового заклинания.
                </GuideRule>
                <GuideRule title="13% · древняя реликвия">
                  Начиная с <b>опасности 3</b> появляется шанс найти <b>Осколок древней реликвии</b>.
                  В руинах опасности 8+ иногда выпадает сразу два осколка.
                </GuideRule>
                <GuideRule title="5% · запечатанный реликварий">
                  Доступен с <b>опасности 6</b>. Даёт 2 Осколка древней реликвии, а на опасности 9–10 — 3,
                  плюс отдельный крупный бонус золота и опыта поверх гарантированной награды.
                </GuideRule>
                <GuideRule title="Низкая опасность не даёт пустой ролл">
                  Если выпал шанс редкой находки, которая ещё недоступна из-за низкой опасности,
                  он превращается в находку свитка. Поэтому исследование руин всегда приносит полноценный результат.
                </GuideRule>
              </div>

              <article className="panel guide-callout">
                <span className="eyebrow">ВАЖНО</span>
                <h3>Каждые руины дают награду только один раз персонажу</h3>
                <p>
                  После завершения они получают статус <b>«исследовано»</b>. Это позволяет делать награды ощутимыми:
                  руины не превращаются в бесконечную ферму золота, опыта или Осколков древней реликвии.
                </p>
              </article>
            </>
          )}
