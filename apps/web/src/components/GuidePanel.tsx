...
const sections: Array<{ id: GuideSection; label: string; description: string }> = [
  ...
  { id: 'effects', label: 'Баффы и дебаффы', description: 'Статусы и особые эффекты' },
  ...
  { id: 'dungeons', label: 'Подземелья', description: 'Награды, истощение и пати' },
]
        ...
        <div className="guide-content">
          ...
          {!loading && catalog && section === 'effects' && (
            <>
              <GuideHeading
                eyebrow="ЭФФЕКТЫ"
                title="Баффы, дебаффы и особые механики"
              ...
              <article className="panel guide-bloodshed">
                ...
                <h3>Что именно делает Кровопролитие</h3>
                <p>
                  Каждый успешный физический хит оружием с Кровопролитием отдельно бросает шанс наложить стак.
                  Это важно для многоударных атак: дополнительные попадания тоже могут дать стаки.
                </p>
                ...
                <p className="muted">
                  Все накопленные стаки взрываются в начале хода цели и после срабатывания сбрасываются.
                  Расчёт идёт последовательно от оставшегося текущего здоровья.
                </p>
              </article>
          ...
          {!loading && catalog && section === 'blacksmith' && (
            <>
              ...
              <article className="panel guide-callout">
                ...
                <h3>Заточка +6 и Пробуждение V — это не одно и то же</h3>
                <p>
                  +6 даёт <b>+18% к базовому урону оружия</b>. Пробуждение V отдельно пять раз усиливает
                  специальную механику предмета. Если у оружия нет специального направления пробуждения,
                  оно получает <b>+5% физического урона оружием</b>.
                </p>
              </article>
            </>
          )}
          ...
          {!loading && catalog && section === 'dungeons' && (
            <>
              <GuideHeading
                eyebrow="ПОДЗЕМЕЛЬЯ"
                title="Повторные зачистки и истощение"
                text="Система позволяет повторять любимый данж, но не даёт бесконечно получать полную награду из одного места."
              />
              ...
              <div className="guide-rule-grid">
                <GuideRule title="После 30-й попытки">
                  Соло-вход в конкретный данж блокируется на <b>20 часов</b>. После окончания истощения
                  цикл штрафов начинается заново.
                </GuideRule>
                <GuideRule title="Истощённый игрок в пати">
                  Войти вместе с группой можно, но истощённый персонаж получает <b>0 XP, 0 золота и 0 личного лута</b>.
                  Остальные участники получают свои обычные награды.
                </GuideRule>
                <GuideRule title="Автобой">
                  Автобой использует те же награды и то же истощение, что ручное прохождение.
                  Он не создаёт отдельный источник опыта или золота.
                </GuideRule>
